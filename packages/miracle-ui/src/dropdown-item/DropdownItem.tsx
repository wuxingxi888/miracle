    ...
    setup(props, { emit, slots, attrs }) {
        ...
        if (!parent) {
            if (process.env.NODE_ENV !== 'production') {
                console.error(
                    '[Miracle] <DropdownItem> must be a child component of <DropdownMenu>.',
                );
            }
            return;
        }

        const getEmitter = (name: 'open' | 'close' | 'opened') => () =>
            emit(name);
        ...
        return () => {
            if (props.teleport) {
                return (
                    <Teleport to={props.teleport}>{renderContent()}</Teleport>
                );
            }
