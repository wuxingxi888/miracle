...
test('should emit change event when active option changed', async () => {
    ...
    await later();
    wrapper.find('.mi-cascader__option').trigger('click');
    ...
    await later();
    wrapper
        .findAll('.mi-cascader__options')[1]
        .find('.mi-cascader__option')
        .trigger('click');
...
test('should emit close event when close icon is clicked', () => {
    const wrapper = mount(Cascader);
    wrapper.find('.mi-cascader__close-icon').trigger('click');
...
test('should not render close icon when closeable is false', () => {
    ...
    expect(wrapper.find('.mi-cascader__close-icon').exists()).toBeFalsy();
});
...
test('should change close icon when using close-icon prop', () => {
    ...
    expect(wrapper.find('.mi-cascader__close-icon').html()).toMatchSnapshot();
});
...
test('should render title slot correctly', () => {
    ...
    expect(wrapper.find('.mi-cascader__title').html()).toMatchSnapshot();
});
...
test('should render option slot correctly', async () => {
    ...
    await later();
    expect(wrapper.find('.mi-cascader__option').html()).toMatchSnapshot();
});
...
test('should select correct option when value changed', async () => {
    ...
    await later();
    const selectedOptions = wrapper.findAll('.mi-cascader__option--selected');
...
test('should reset selected options when value is set to empty', async () => {
    ...
    expect(wrapper.find('.mi-cascader__option--selected').exists()).toBeFalsy();
});
...
test('should update tabs when previous tab is clicked', async () => {
    ...
    await later();
    wrapper.findAll('.mi-cascader__option')[1].trigger('click');
    await later();
...
test('should emit clickTab event when a tab is clicked', async () => {
    ...
    const tabs = wrapper.findAll('.mi-tab');
...
test('should allow to custom the className of option', async () => {
    ...
    await later();
    const option = wrapper.find('.mi-cascader__option');
...
test('should render options-top、options-bottom slots correctly', async () => {
    const wrapper = mount(Cascader, {
        slots: {
            ...
            'options-bottom': ({ tabIndex }) =>
                `Bottom, tab index: ${tabIndex}`,
        },
        props: {
            options,
        },
    ...
    await later();
    await wrapper
        .findAll('.mi-cascader__options')[0]
        .find('.mi-cascader__option')
        .trigger('click');

    expect(wrapper.find('.mi-tab__panel').html()).toMatchSnapshot();
});
...
test('should not render header when show-header prop is false', async () => {
    ...
    const header = wrapper.find('.mi-cascader__header');
