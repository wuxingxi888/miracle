...
test('should emit select event when clicking the action', async () => {
    ...
    await later();
    await wrapper.find('.mi-popover__action').trigger('click');
...
test('should not emit select event when the action is disabled', () => {
    ...
    wrapper.find('.mi-popover__action').trigger('click');
...
test('should allow to custom the className of action', () => {
    ...
    expect(wrapper.find('.mi-popover__action').html()).toMatchSnapshot();
});
...
test('should allow to custom the color of action test', () => {
    ...
    expect(wrapper.find('.mi-popover__action').style.color).toEqual('red');
});
...
test('should close popover when touch outside content', async () => {
    ...
    const popover = root.querySelector('.mi-popover');
...
test('should allow to hide arrow', () => {
    ...
    expect(wrapper.find('.mi-popover__arrow').exists()).toBeFalsy();
});
...
test('should add "mi-popover__content--horizontal" class when actions-direction prop is horizontal', () => {
    const wrapper = mount(Popover, {
        props: {
            show: true,
            actions: baseActions,
    ...
    expect(wrapper.find('.mi-popover__content').classes()).toContain(
        'mi-popover__content--horizontal',
    );
});
