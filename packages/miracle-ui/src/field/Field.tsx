...
export default defineComponent({
    ...
    setup(props, { emit, slots }) {
                ...
                (promise, rule) =>
                    promise.then(() => {
                        ...
                        if (rule.validator) {
                            if (
                                isEmptyValue(value) &&
                                rule.validateEmpty === false
                            ) {
                                return;
                            }

                            return runRuleValidator(value, rule).then(
                                (result) => {
                                    if (result && typeof result === 'string') {
                                        ...
                                        state.status = 'failed';
                                        state.validateMessage = getRuleMessage(
                                            value,
                                            rule,
                                        );
                                    }
        ...
        const validateWithTrigger = (trigger: FieldValidateTrigger) => {
            if (form && props.rules) {
                ...
                const defaultTrigger =
                    toArray(validateTrigger).includes(trigger);
        ...
        // see: https://github.com/wuxingxi888/miracle/issues/5033
        ...
        const updateValue = (
            ...
            if (props.formatter && trigger === props.formatTrigger) {
                ...
                if (inputRef.value && state.focused) {
                    ...
                    formatterDiffLen =
                        getStringLength(formatter(bcoVal)) -
                        getStringLength(bcoVal);
                }
            }
