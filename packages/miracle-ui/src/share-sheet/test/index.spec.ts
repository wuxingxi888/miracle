...
test('should render cancel text when using cancel-text prop', async () => {
  ...
  expect(wrapper.find('.mi-share-sheet__cancel').html()).toMatchSnapshot();

  await wrapper.setProps({ cancelText: '' });
  expect(wrapper.find('.mi-share-sheet__cancel').exists()).toBeFalsy();
});
...
test('should render description when using description prop', async () => {
  ...
  expect(wrapper.find('.mi-share-sheet__description').html()).toMatchSnapshot();

  await wrapper.setProps({ description: '' });
  expect(wrapper.find('.mi-share-sheet__description').exists()).toBeFalsy();
});
...
test('should allow to custom the className of option', () => {
  ...
  const option = wrapper.find('.mi-share-sheet__option').element;
...
test('should emit select event when an option is clicked', () => {
  ...
  wrapper.find('.mi-share-sheet__option').trigger('click');
...
test('should emit cancel event when the cancel button is clicked', () => {
  ...
  wrapper.find('.mi-share-sheet__cancel').trigger('click');
...
test('should emit clickOverlay event when overlay is clicked', async () => {
  ...
  const overlay = root.querySelector('.mi-overlay')!;
...
test('should allow to custom the icon of option', () => {
  ...
  expect(
    wrapper.element.querySelectorAll('.mi-share-sheet__option').length,
  ).toEqual(2);
});
