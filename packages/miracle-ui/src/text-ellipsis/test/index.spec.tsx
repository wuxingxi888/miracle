...
const content =
    'Miracle is a lightweight, customizable mobile component library that was open sourced in 2017. Currently Miracle officially provides Vue 2 version, Vue 3 version and WeChat applet version, and the community team maintains React version and Alipay applet version.';
    ...
    Object.defineProperty(HTMLElement.prototype, 'offsetHeight', {
        get() {
            if (this.innerText.includes('...')) {
                const row = Math.ceil(
                    (this.innerText.replace(/\.\.\./g, '中').length /
                        content.length) *
                        4,
                );
...
test('Expand and Collapse should be work', async () => {
    ...
    await wrapper.find('.mi-text-ellipsis__action').trigger('click');
...
test('text not exceeded', async () => {
    ...
    const shortContent = 'Miracle is a component library';
    const wrapper = mount(TextEllipsis, {
        attachTo: document.body,
        props: {
...
// https://github.com/wuxingxi888/miracle/issues/12445
