...
test('should drag adsorption effect when anchors props is [100, 200, 400]', async () => {
  ...
  // drag 10
  await triggerDrag(wrapper.find('.mi-floating-panel__header'), 0, 10);
  await later();
  expect((wrapper.element as HTMLDivElement).style.transform).toContain(
    '-100px',
  );
  ...
  // drag -49
  await triggerDrag(wrapper.find('.mi-floating-panel__header'), 0, -49);
  await later();
  expect((wrapper.element as HTMLDivElement).style.transform).toContain(
    '-100px',
  );

  await triggerDrag(wrapper.find('.mi-floating-panel__header'), 0, -199);
  await later();
  expect((wrapper.element as HTMLDivElement).style.transform).toContain(
    '-200px',
  );
  ...
  // drag -300
  await triggerDrag(wrapper.find('.mi-floating-panel__header'), 0, -200);
  await later();
  expect((wrapper.element as HTMLDivElement).style.transform).toContain(
    '-400px',
  );
  ...
  // drag -500
  await triggerDrag(wrapper.find('.mi-floating-panel__header'), 0, -500);
  await later();
  expect((wrapper.element as HTMLDivElement).style.transform).toContain(
    '-400px',
  );
...
test('should only drag header when allowDraggingContent is false', async () => {
  ...
  await triggerDrag(wrapper.find('.mi-floating-panel__content'), 0, -199);
  await later();
