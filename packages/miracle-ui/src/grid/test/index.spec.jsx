...
test('should change icon size when using icon-size prop', () => {
    ...
    expect(wrapper.find('.mi-grid-item__icon').style.fontSize).toEqual('10px');
});
...
test('should change icon color when using icon-color prop', () => {
    ...
    expect(wrapper.find('.mi-grid-item__icon').style.color).toEqual('red');
});
...
test('should render ".mi-grid-item__content--reverse" class when using reverse prop', () => {
    const wrapper = mount({
        render() {
            return (
                <Grid reverse>
    ...
    expect(wrapper.find('.mi-grid-item__content').classes()).toContain(
        'mi-grid-item__content--reverse',
    );
});
