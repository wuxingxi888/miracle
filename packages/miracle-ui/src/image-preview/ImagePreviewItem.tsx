...
export default defineComponent({
    ...
    setup(props, { emit, slots }) {
        ...
        const imageStyle = computed(() => {
            const { scale, moveX, moveY, moving, zooming, initializing } =
                state;
            const style: CSSProperties = {
                transitionDuration:
                    zooming || moving || initializing ? '0s' : '.3s',
            };
        ...
        const maxMoveX = computed(() => {
            if (state.imageRatio) {
                ...
                return Math.max(
                    0,
                    (state.scale * displayWidth - rootWidth) / 2,
                );
            }
        ...
        const maxMoveY = computed(() => {
            if (state.imageRatio) {
                ...
                return Math.max(
                    0,
                    (state.scale * displayHeight - rootHeight) / 2,
                );
            }
        ...
        const setScale = (scale: number, center?: { x: number; y: number }) => {
            ...
            if (scale !== state.scale) {
                ...
                if (center) {
                    ...
                    const moveX =
                        state.moveX -
                        (center.x - imageRect.left - origin.x) * (ratio - 1);
                    const moveY =
                        state.moveY -
                        (center.y - imageRect.top - origin.y) * (ratio - 1);
