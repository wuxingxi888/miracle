...
test('support controlled mode to set active-tab', async () => {
    ...
    expect(tabs[0]?.classes()).toContain('mi-tab--active');

    activeTab.value = 1;
    await later();
    expect(tabs[1]?.classes()).toContain('mi-tab--active');
});
...
test('should render correctly with v-for', async () => {
    ...
    const items = wrapper.findAll('.mi-swipe-item');

    items.forEach((item) => {
        const picker = item.find('.mi-picker');
