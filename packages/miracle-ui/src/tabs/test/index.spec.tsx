...
test('should not render zero badge when show-zero-badge prop is false', async () => {
    ...
    await later();
    expect(wrapper.findAll('.mi-badge')).toHaveLength(1);
});
...
test('swipe switch tab after swiping tab content', async () => {
    ...
    const content = wrapper.find('.mi-swipe__track');
...
test('should allow to disable lazy-render prop', async () => {
    ...
    expect(wrapper.find('.mi-tabs__content').html()).toMatchSnapshot();
...
test('should render dot prop correctly', async () => {
    ...
    await later();
    const firstTab = wrapper.find('.mi-tab');
    expect(firstTab.find('.mi-badge--dot').exists()).toBeTruthy();
});
...
test('should render badge prop correctly', async () => {
    ...
    await later();
    expect(wrapper.find('.mi-tab').html()).toMatchSnapshot();
});
...
test('should allot to hide bottom border by border prop', async () => {
    ...
    expect(wrapper.find('.mi-hairline--top-bottom').exists()).toBeTruthy();

    await wrapper.setProps({ border: false });
    expect(wrapper.find('.mi-hairline--top-bottom').exists()).toBeFalsy();
});
...
test('should render nav-left、nav-right slot correctly', async () => {
    ...
    await later();
    expect(wrapper.find('.mi-tabs__wrap').html()).toMatchSnapshot();
});
...
test('should not render header when showHeader is false', async () => {
    ...
    await later();
    const tabs = wrapper.findAll('.mi-tabs__wrap,.mi-sticky');
...
test('should re-render when line-width or line-height changed', async () => {
    ...
    await later();
    const line = wrapper.find('.mi-tabs__line');
