...
test('should emit change event after the page is changed', async () => {
    ...
    await wrapper
        .findAll('.mi-pagination__item--page button')[2]
        .trigger('click');
    expect(wrapper.emitted('change')).toEqual([[3]]);

    await wrapper.find('.mi-pagination__item--prev button').trigger('click');
    expect(wrapper.emitted('change')).toEqual([[3], [2]]);

    await wrapper.find('.mi-pagination__item--next button').trigger('click');
...
test('should allow to hide prev button and next button', () => {
    ...
    expect(wrapper.find('.mi-pagination__item--prev').exists()).toBeFalsy();
    expect(wrapper.find('.mi-pagination__item--next').exists()).toBeFalsy();
});
