...
test('show dropdown item', async () => {
    ...
    const titles = wrapper.findAll('.mi-dropdown-menu__title');
...
test('click option', async () => {
    ...
    const options = wrapper.findAll('.mi-dropdown-item .mi-cell');
...
test('disable dropdown item', async () => {
    ...
    await later();
    const title = wrapper.find('.mi-dropdown-menu__title');
        ...
        const wrapper = mount({
            setup() {
                ...
                onMounted(async () => {
                    ...
                    expect(
                        wrapper.find('.mi-dropdown-item__content').style
                            .display,
                    ).toEqual('');
...
test('DropdownItem should inherit attrs when using teleport prop', async () => {
    ...
    await later();
    const item = root.querySelector('.mi-dropdown-item');
...
test('scrolling is allowed when the number of items exceeds the threshold', async () => {
    ...
    const bar = wrapper.find('.mi-dropdown-menu__bar');
    expect(bar.classes()).not.toContain('mi-dropdown-menu__bar--scrollable');
    itemCounts.value = 5;
    await later();
    expect(bar.classes()).toContain('mi-dropdown-menu__bar--scrollable');
});
...
test('auto-locate prop', async () => {
    ...
    const item = wrapper.find('.mi-dropdown-item');
    const offsetParent = {
