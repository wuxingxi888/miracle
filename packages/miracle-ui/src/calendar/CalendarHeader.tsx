...
export default defineComponent({
    ...
    setup(props, { slots, emit }) {
        ...
        const renderAction = (isNext?: boolean) => {
            ...
            const yearIconName = isNext
                ? 'arrow-double-right'
                : 'arrow-double-left';

            const onMonthChange = () =>
                onPanelChange(
                    (isNext ? getNextMonth : getPrevMonth)(props.date!),
                );
            const onYearChange = () =>
                onPanelChange(
                    (isNext ? getNextYear : getPrevYear)(props.date!),
                );
            ...
            return isNext
                ? [MonthAction, YearAction]
                : [YearAction, MonthAction];
        };
                ...
                return (
                    ...
                    >
                        {canSwitch
                            ? [
                                  renderAction(),
                                  <div class={bem('header-subtitle-text')}>
                                      {title}
                                  </div>,
                                  renderAction(true),
                              ]
