    ...
    setup(props, { emit, slots }) {
        ...
        const getInitialDate = (defaultDate = props.defaultDate) => {
            ...
            if (type === 'range') {
                ...
                if (
                    ...
                    compareDay(defaultDate[0], now) === 1
                ) {
                    defaultDate = [];
                }
        ...
        const onScroll = () => {
            ...
            months.value.forEach((month, index) => {
                const visible =
                    index >= visibleRange[0] - 1 &&
                    index <= visibleRange[1] + 1;
                ...
                raf(() => {
                    months.value.some((month, index) => {
                        if (compareMonth(month, targetDate) === 0) {
                            if (bodyRef.value) {
                                monthRefs.value[index].scrollToDate(
                                    bodyRef.value,
                                    targetDate,
                                );
                            }
        ...
        const init = () => {
            ...
            if (!canSwitch.value) {
                raf(() => {
                    ...
                    // https://github.com/wuxingxi888/miracle/issues/5640
        ...
        const onConfirm = () =>
            emit(
                'confirm',
                currentDate.value ?? cloneDates(currentDate.value!),
            );
        ...
        const onClickDay = (item: CalendarDayItem) => {
            ...
            if (type === 'range') {
                ...
                if (selectedIndex !== -1) {
                    ...
                    showToast(
                        props.rangePrompt || t('rangePrompt', props.maxRange),
                    );
                } else {
                    select([...dates, date]);
                }
        ...
        const renderMonth = (date: Date, index: number) => {
            ...
            return (
                <CalendarMonth
                    ...
                    ref={
                        canSwitch.value ? currentMonthRef : setMonthRefs(index)
                    }
                    date={date}
