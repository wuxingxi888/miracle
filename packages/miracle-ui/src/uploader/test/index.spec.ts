...
test('disabled', async () => {
  ...
  await later();
  const input = wrapper.find<HTMLInputElement>('.mi-uploader__input');
...
test('render upload-text', () => {
  ...
  const text = wrapper.find<HTMLSpanElement>('.mi-uploader__upload-text');
...
test('should allow to custom size by preview-size prop', async () => {
  ...
  const image = wrapper.find('.mi-uploader__file');
  expect(image.style.width).toEqual('30px');
  expect(image.style.height).toEqual('30px');

  const upload = wrapper.find('.mi-uploader__upload');
...
test('deletable prop', async () => {
  ...
  expect(wrapper.find('.mi-uploader__preview-delete').exists()).toBeTruthy();

  await wrapper.setProps({ deletable: false });
  expect(wrapper.find('.mi-uploader__preview-delete').exists()).toBeFalsy();
});
...
test('delete preview image', () => {
  ...
  wrapper.find('.mi-uploader__preview-delete').trigger('click');
...
test('click to preview image', async () => {
  ...
  wrapper.find('.mi-image').trigger('click');
  await later();
  expect(document.querySelector('.mi-image-preview')).toBeFalsy();

  await wrapper.setProps({ previewFullImage: true });
  wrapper.find('.mi-image').trigger('click');
  await later();
  expect(document.querySelector('.mi-image-preview')).toBeTruthy();

  const images = document.querySelectorAll<HTMLImageElement>(
    '.mi-image-preview .mi-image-preview__image',
  );
...
test('preview-options prop', async () => {
  ...
  const closeIcon = document.querySelectorAll('.mi-image-preview__close-icon');
...
test('closePreview event', async () => {
  ...
  await later();
  await wrapper.find('.mi-image').trigger('click');

  const preview = document.querySelector<HTMLDivElement>('.mi-image-preview');
...
test('show-upload prop', async () => {
  ...
  expect(wrapper.find('.mi-uploader__upload').exists()).toBeTruthy();
  await wrapper.setProps({ showUpload: false });
  expect(wrapper.find('.mi-uploader__upload').style.display).toBe('none');
});
...
test('should render preview-cover slot correctly', async () => {
  ...
  expect(wrapper.find('.mi-uploader__preview-cover').html()).toMatchSnapshot();
});
...
test('should render preview-delete slot correctly', async () => {
  ...
  expect(wrapper.find('.mi-uploader__preview-delete').html()).toMatchSnapshot();
});
...
test('should emit clickUpload event when upload area is clicked', async () => {
  const wrapper = mount(Uploader);
  wrapper.find('.mi-uploader__upload').trigger('click');
