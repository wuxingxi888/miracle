...
test('should skip validator if validateEmpty is false in rules prop', async () => {
    const onFailed = vi.fn();
    const rules: FieldRule[] = [
        { validator: () => false, validateEmpty: false },
    ];
    const wrapper = mount({
        render() {
            return (
...
test('should trigger validate after blurring when validate-trigger prop is onBlur', async () => {
    ...
    expect(wrapper.find('.mi-field__error-message').exists()).toBeFalsy();

    await input.trigger('blur');
    expect(wrapper.find('.mi-field__error-message').exists()).toBeTruthy();
});
...
test('should allow to custom trigger in rules', async () => {
    ...
    await later();
    expect(
        wrapper.element.querySelectorAll('.mi-field__error-message').length,
    ).toEqual(2);
});
...
test('should allow to control the display of error status by show-error prop', async () => {
    ...
    await submitForm(wrapper);
    expect(wrapper.find('.mi-field--error').exists()).toBeFalsy();

    await wrapper.setData({ showError: true });
    await submitForm(wrapper);
    expect(wrapper.find('.mi-field--error').exists()).toBeTruthy();
});
