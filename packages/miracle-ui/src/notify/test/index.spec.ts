...
test('should render Notify correctly', async () => {
    showNotify('test');
    await later();
    expect(document.querySelector('.mi-notify')).toMatchSnapshot();
});

test('should add "mi-notify--success" class when type is success', async () => {
    showNotify({
        message: 'test',
        type: 'success',
    });
    ...
    await later();
    const notify = document.querySelector('.mi-notify') as HTMLElement;
    expect(notify.classList.contains('mi-notify--success')).toBeTruthy();
});
...
test('should change default options after calling setDefaultOptions method', async () => {
    ...
    showNotify({});
    await later();
    const notify2 = document.querySelector('.mi-notify') as HTMLElement;
...
test('should align to bottom when position option is bottom', async () => {
    ...
    await later();
    const notify = document.querySelector('.mi-notify') as HTMLElement;
    expect(notify.classList.contains('mi-popup--bottom')).toBeTruthy();
});
