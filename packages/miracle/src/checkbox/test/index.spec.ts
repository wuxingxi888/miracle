...
test('should emit "update:modelValue" event when checkbox icon is clicked', async () => {
  ...
  const icon = wrapper.find('.mi-checkbox__icon');
  icon.trigger('click');
...
test('should not emit "update:modelValue" event when checkbox icon is disabled and clicked', () => {
  ...
  wrapper.find('.mi-checkbox__icon').trigger('click');
  expect(wrapper.emitted('update:modelValue')).toBeFalsy();
});

test('should render "mi-checkbox--label-disabled" class when using label-disabled prop', () => {
  const wrapper = mount(Checkbox, {
    props: {
      labelDisabled: true,
    },
  ...
  expect(wrapper.classes()).toContain('mi-checkbox--label-disabled');
});
...
test('should emit "update:modelValue" event when label is clicked', () => {
  ...
  const icon = wrapper.find('.mi-checkbox__label');
  icon.trigger('click');
  expect(wrapper.emitted('update:modelValue')![0]).toEqual([true]);
});
...
test('should render icon slot correctly', async () => {
  ...
  expect(wrapper.find('.mi-checkbox__icon').html()).toMatchSnapshot();
});
