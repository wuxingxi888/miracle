...
test('resetValidation method - reset all fields', async () => {
  ...
  try {
    ...
    await later();
    const errors = form.findAll('.mi-field__error-message');
...
test('resetValidation method - reset one field', async () => {
  ...
  try {
    ...
    await later();
    expect(form.findAll('.mi-field__error-message')).toHaveLength(1);

    formRef.value?.resetValidation('B');
    await later();
    expect(form.findAll('.mi-field__error-message')).toHaveLength(0);
  }
});
