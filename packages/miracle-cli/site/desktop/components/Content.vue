<template>
  <div
    :class="['mi-doc-content', `mi-doc-content--${currentPage}`]"
    @click="onClick"
  >
    <slot />
  </div>
...
export default {
  ...
  methods: {
    ...
    copyAction() {
      const codeBoxes = document.querySelectorAll('.mi-doc-card pre code');
...
<style lang="less">
.mi-doc-card {
  margin-bottom: var(--mi-doc-padding);
  ...
  > table {
    ...
    td {
      padding: 8px;
      border-top: 1px solid var(--mi-doc-border-color);
    ...
    em {
      ...
      color: var(--mi-doc-green);
      font-size: 14px;
      font-family: var(--mi-doc-code-font-family);
      font-style: normal;
      max-width: 300px;
      -webkit-font-smoothing: auto;
    }
  ...
  > blockquote {
    ...
    font-size: 15px;
    line-height: 26px;
    color: var(--mi-doc-blockquote-color);
    background-color: var(--mi-doc-blockquote-background);
    border-radius: var(--mi-doc-border-radius);
  }
...
.mi-doc-content {
  ...
  .mi-doc-markdown-body {
    padding: var(--mi-doc-padding);
    overflow: hidden;
