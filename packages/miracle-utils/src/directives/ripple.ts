...
function getRippleOptions(modifiers: Record<string, boolean>): RippleOptions {
    ...
    if ('event' in modifiers) {
        options.event =
            Object.keys(modifiers).find((modifier) => modifier !== 'event') ||
            defaultRippleOptions.event;
    }
...
function clearRipple(el: HTMLElement) {
    const rippleContainer = el.querySelector(
        '.ripple-container',
    ) as HTMLElement;
    if (rippleContainer) {
...
// 添加 Ripple 清除事件监听器
function addRippleClearEventListeners(
    el: HTMLElement,
