...
export function getBrowserInfo(): BrowserInfo {
    ...
    const supporter =
        supporters.find((sup) => testUa(sup.regex))?.name || 'unknown';
    const supporterVs =
        testVs(new RegExp(`${supporter}/[\\d._]+`, 'g')) || 'unknown';
...
export function getDetectionMethods() {
    ...
    return {
        ...
        // 平台检测
        mobilePlatform:
            /iPhone|iPad|iPod|Android|BlackBerry|Windows Phone/i.test(platform),
